<template>
  <div>
    <div class="nav-container flex flex-between flex-align-center">
      <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="219" height="29" viewBox="0 0 219 29"><g><g><image width="219" height="29" xlink:href="data:image/png;base64,"/></g></g></svg>
      <div class="flex flex-between flex-align-center h-100 links flex">
        <div class="single-link flex flex-align-center active"><a href="javascript:" class="flex flex-align-center">home</a> </div>
        <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">sport</a> </div>
        <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">livebetting</a> </div>
        <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">poker</a> </div>
        <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">casino</a> </div>
        <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">live casino</a> </div>
        <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">games</a> </div>
        <div class="holder-menu-ital">
          <img src="../assets/imgs/menu-circular-button.png" @click="burgerHandler" class="hamburger">
          <img src="../assets/imgs/italy_(3).png" class="lang-menu" @click="langListHandler">
        </div>
      </div>
    </div>
    <div class="clone-links" :class="{toggle: burger}">
      <div class="single-link flex flex-align-center active"><a href="javascript:" class="flex flex-align-center">home</a> </div>
      <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">sport</a> </div>
      <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">livebetting</a> </div>
      <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">poker</a> </div>
      <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">casino</a> </div>
      <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">live casino</a> </div>
      <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">games</a> </div>
    </div>
    <div class="lang-list" :class="{toggle: langList}">
      <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">English</a> </div>
      <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">Albanian</a> </div>
      <div class="single-link flex flex-align-center active"><a href="javascript:" class="flex flex-align-center">Italian</a> </div>
      <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">Chinese</a> </div>
      <div class="single-link flex flex-align-center"><a href="javascript:" class="flex flex-align-center">Spanish</a> </div>
    </div>

    <div class="border"></div>
  </div>
</template>

<script>
  export default {
    name: "NavLinksComponent",
    data: () => ({
      burger: false,
      langList: false
    }),
    methods: {
      burgerHandler: function () {
        if (this.langList) {
          this.langList = false;
        }
        this.burger = !this.burger
      },
      langListHandler: function () {
        if (this.burger) {
          this.burger = false;
        }
        this.langList = !this.langList;
      }
    }
  }
</script>

<style scoped lang="scss">
  @import "../assets/scss/const";
  @import "../assets/scss/vars";

  .nav-container {
    padding: 0 0 1px 50px;
    background-color: $body-bg;
    height: 60px;
    .links:hover {
      .single-link.active {
        transform: none;
        background-color: unset;
        box-shadow: none;
        a {
          color: $nav-links-cl;
          transform: none;
        }
      }
      .single-link:hover {
        transform: skew(-20deg);
        height: 100%;
        padding: 0 10px;
        background-color: #1d1d1d;
        box-shadow: inset 0 0 10px #000000;
        a {
          color: $nav-links-cl-active;
          transform: skew(20deg);
        }
      }
    }

    .single-link.active {
      transform: skew(-20deg);
      height: 100%;
      padding: 0 10px;
      background-color: #1d1d1d;
      box-shadow: inset 0 0 10px #000000;
      a {
        color: $nav-links-cl-active;
        transform: skew(20deg);
      }
    }

    .single-link {
      height: 100%;
      padding: 0 10px;
      @include media (cs, 0, 1200px) {
        padding: 0 7px;
      }
    }
    a {
      height: 100%;
      color: $nav-links-cl;
      font-family: $nav-font-family;
      font-size: 0.857em;
      font-weight: 400;
      text-transform: uppercase;
      text-decoration: none;
      margin: 0;
    }
    .holder-menu-ital {
      background-image: url("../assets/imgs/Layer_1.png");
      height: 100%;
      background-repeat: no-repeat;
      background-size: cover;
      width: 108px;
      display: flex;
      justify-content: center;
      align-items: center;
      & img:nth-child(1) {
        padding: 0 10px;

      }
    }

    .links {
      .single-link {
        display: none;
      }
      @include media(cs, 1200px) {
        .hamburger {
          display: none;
        }
        .single-link {
          display: flex;
        }
      }

    }
  }

  .clone-links, .lang-list {
    display: none;
    &.lang-list.toggle {
      display: flex;
    }
    width: 200px;
    position: absolute;
    right: 0;
    z-index: 99;
    background-color: #1d1d1d;
    flex-flow: column;
    align-items: center;
    border: 3px solid #1d1d1d;
    margin-bottom: 5px;
    &:hover {
      .single-link.active {
        transform: none;
        background-color: unset;
        box-shadow: none;
        a {
          color: $nav-links-cl;
          transform: none;
        }
      }
      .single-link:hover {
        transform: skew(-20deg);
        height: 100%;
        padding: 0 10px;
        background-color: #1d1d1d;
        box-shadow: inset 0 0 10px #000000;
        a {
          color: $nav-links-cl-active;
          transform: skew(20deg);
        }
      }
    }

    @include media(cs, 0, 1200px) {
      &.toggle {
        display: flex;
      }
    }

    .single-link {
      margin-bottom: 5px;
    }
    a {
      color: #b4d8f7;
      font-family: $font-rajdhani;
      font-size: 0.857em;
      font-weight: 400;
      text-transform: uppercase;
      text-decoration: none;
      cursor: pointer;
    }
    .single-link:hover {
      transform: skew(-20deg);
      height: 100%;
      padding: 0 10px;
      background-color: #1d1d1d;
      box-shadow: inset 0 0 10px #000000;
      a {
        color: $nav-links-cl-active;
        transform: skew(20deg);
      }
    }
    .active {
      transform: skew(-20deg);
      height: 100%;
      padding: 0 10px;
      background-color: #1d1d1d;
      box-shadow: inset 0 0 10px #000000;
      a {
        color: $nav-links-cl-active;
        transform: skew(20deg);
      }
    }
  }
  .border {
    width: 100%;
    height: 3px;
    background-image: linear-gradient(to right, #ff6000 , #ff6000, #ffb700 , #ff6000 , #ff6000);
  }
</style>
